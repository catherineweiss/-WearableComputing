...
columnsToSelect <- c("activity", "subject", 
                     grep("-mean()", names(data), value=TRUE, fixed=TRUE),
                     grep("-std()", names(data), value=TRUE, fixed=TRUE) ) 
...
variableNames <- gsub("Acc", "Acceleration", variableNames, fixed=TRUE)
...
variableNames <- gsub("Gyro", "AngularVelocity", variableNames, fixed=TRUE)
